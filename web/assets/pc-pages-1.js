...
function renderOverview() {
  const L = (window.LIVE && window.LIVE.overview) || null;
  const sParks = L ? L.totalParks : 2;
  const sShops = L ? L.totalShops : 5;
  const sUsers = L ? L.totalUsers : 2837;
  const sSales = L ? Number(L.todaySales) : 1286.50;
  return `
    <div class="page-header">
      <div class="page-title">园区总览</div>
      <div class="page-subtitle">欢迎回来，${(typeof YFSC!=='undefined'&&YFSC.user&&YFSC.user.name)||ROLES[VIEW_TO_ROLE[APP.view]].user}。以下为${L?'实时':'全局'}经营概况。</div>
    </div>

    <div class="stat-grid c4">
      ${statCard('总园区数', sParks, {unit:'', icon:antStat('building'), color:'#1677ff', extra:'营业中 '+sParks+' · 筹备中 0'})}
      ${statCard('总商户数', sShops, {unit:'', icon:antStat('shop'), color:'#52c41a', extra:L?'实时统计':'餐饮 3 · 零售 1 · 体验 1'})}
      ${statCard('总用户数', sUsers, {unit:'', icon:antStat('user'), color:'#722ed1', delta:'+'+(L?L.todayNewUsers:12), deltaType:'up', extra:'今日新增'})}
      ${statCard('今日总销售额', sSales, {icon:antStat('dollar'), color:'#fa8c16', delta:'12.5%', deltaType:'up', extra:L?'实时':'较昨日'})}
    </div>
